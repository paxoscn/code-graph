...
public class ConstructorDefinition extends Process {
    ...
    public void build(ParseTree parseTree) {
        for (int i = 0, c = parseTree.getChildCount(); i < c; i++) {
            ...
            } else if (child instanceof JavaParser.BlockContext) {
                JavaParser.BlockContext blockContext = (JavaParser.BlockContext) child;
